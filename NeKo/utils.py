...
def format_connectivity_guidance() -> str:
    ...
    response += f"**4. Try a broader path policy:**\n"
    response += f"```\n"
    response += f"create_network(your_genes, path_policy='all_bounded')\n"
    response += f"```\n\n"
...
def format_network_creation_error(error_type: str, genes: list, error_msg: str) -> str:
    ...
    if error_type == "build_failed":
        ...
        response += f"   - Try `path_policy='all_bounded'` for all bounded paths\n"
